...
class User(ndb.Expando):
    ...
    description = ndb.TextProperty('d', indexed=False, default="I am a stranger. I come in peace. Take me to your leader and there will be a massive reward for you in eternity.")
    tags = ndb.StringProperty('t', repeated=True)
    # card_uri = ndb.StringProperty()
    # bank_account_uri = ndb.StringProperty()
...
#            user._add_emails(emails)
    ...
    def get_or_create_by_profile(cls, profile):
        ...
        username = profile.user_info.get('info').get('username')
        return cls._get_or_create(profile.key.id(), emails,
                username=username, email=emails[0]['value'])
